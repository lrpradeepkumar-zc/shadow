...
// FIXED: Settings button selector, added workflow nav, init ApprovalUI, emit approval:ui:refresh
(function() {
    'use strict';
    let state = { tasks: [], groups: [], tags: [], currentView: 'agenda', currentViewType: 'board', selectedTaskId: null, sortBy: 'dueDate', sortDirection: 'desc', searchQuery: '' };
    let dbReady = false;

 function formatDate(ds) { if (!ds) return ''; const d = new Date(ds); const m = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; return d.getDate()+' '+m[d.getMonth()]+' '+d.getFullYear(); }
    function isOverdue(ds) { return ds && new Date(ds) < new Date(); }
    function getDateCategory(ds) { if (!ds) return 'nodate'; const d=new Date(ds),now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),td=new Date(d.getFullYear(),d.getMonth(),d.getDate()),diff=td-today,day=86400000; if(diff<0) return 'delayed'; if(diff===0) return 'today'; if(diff<=7*day) return 'thisweek'; return 'thismonth'; }
 ...
 function renderSidebar() {
         document.getElementById('groupsList').innerHTML = state.groups.filter(g=>g.type!=='personal').map(g=>{ const c=state.tasks.filter(t=>t.group===g.id).length; return '<div class="group-item" data-group="'+g.id+'"><i class="fa-solid fa-chevron-right" style="font-size:10px"></i> '+g.name+(c?'<span class="count">'+c+'</span>':'')+'</div>'; }).join('');
         ...
         document.getElementById('personalCount').textContent = state.tasks.filter(t=>{ const g=state.groups.find(gr=>gr.id===t.group); return g&&g.type==='personal'; }).length;
 }
 ...
 function renderListRow(t) {
         return '<div class="list-row" data-taskid="'+t.id+'"><div class="list-col title-col"><div class="check-circle"></div>'+(t.priority==='High'?'<span class="priority-indicator">!</span> ':'')+t.title+(t.subtasks&&t.subtasks.length?' <span class="subtask-count"><i class="fa-regular fa-square-check"></i> '+t.subtasks.filter(s=>s.done).length+'/'+t.subtasks.length+'</span>':'')+'</div><div class="list-col assignee-col">'+(t.assignee?'<span class="avatar-sm" style="width:22px;height:22px;font-size:10px">'+t.assignee.charAt(0)+'</span> '+t.assignee:'')+'</div><div class="list-col status-col"><span class="status-badge '+statusClass(t.status)+'">'+t.status+'</span></div><div class="list-col due-date-col">'+(t.dueDate?'<i class="fa-regular fa-calendar"></i> '+formatDate(t.dueDate):'')+'</div></div>';
 }

 function renderListView() {
         const lb=document.getElementById('listBody'); let tasks=getFilteredTasks();
         if (state.currentView==='agenda') {
                     const cats={delayed:[],today:[],thisweek:[],thismonth:[]},labels={delayed:'Delayed',today:'Today',thisweek:'This week',thismonth:'This month'},colors={delayed:'#34a853',today:'#ea4335',thisweek:'#e91e63',thismonth:'#4285f4'};
                     tasks.forEach(t=>{const c=getDateCategory(t.dueDate);if(cats[c])cats[c].push(t);});
                     let h=''; Object.entries(cats).forEach(([k,ts])=>{if(ts.length||k==='delayed'){h+='<div class="list-group-header"><div class="check-circle"></div><div class="group-color" style="background:'+colors[k]+'"></div>'+labels[k]+'</div>';ts.forEach(t=>{h+=renderListRow(t);});}}); lb.innerHTML=h;
         } else {
                     const gr={}; tasks.forEach(t=>{const n=t.category||'General';if(!gr[n])gr[n]=[];gr[n].push(t);});
                     let h=''; Object.entries(gr).forEach(([n,ts])=>{h+='<div class="list-group-header"><div class="check-circle"></div><div class="group-color" style="background:#e67e22"></div>'+n+'</div>';ts.forEach(t=>{h+=renderListRow(t);});}); lb.innerHTML=h;
         }
 }
 ...
 function renderView() { if(state.currentViewType==='board'){document.getElementById('boardView').style.display='';document.getElementById('listView').style.display='none';renderBoardView();}else{document.getElementById('boardView').style.display='none';document.getElementById('listView').style.display='';renderListView();} }

 function showTaskDetail(taskId) {
         const task=state.tasks.find(t=>t.id===taskId); if(!task) return; state.selectedTaskId=taskId;
         ...
         // FIX: Trigger approval UI refresh
        if(typeof ApprovalWorkflow!=='undefined'){ApprovalWorkflow.emit('approval:ui:refresh',{taskId:taskId});}
 }
    function hideTaskDetail(){document.getElementById('taskDetailPanel').style.display='none';state.selectedTaskId=null;}

 // Nav items
 document.querySelectorAll('.nav-item').forEach(item=>{item.addEventListener('click',function(){document.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));this.classList.add('active');state.currentView=this.dataset.view;renderView();});});
    document.querySelectorAll('.view-tab').forEach(tab=>{tab.addEventListener('click',function(){document.querySelectorAll('.view-tab').forEach(t=>t.classList.remove('active'));this.classList.add('active');state.currentViewType=this.dataset.viewtype;renderView();});});
 ...
 // Task clicks
 document.getElementById('boardColumns').addEventListener('click',function(e){const c=e.target.closest('.task-card');if(c)showTaskDetail(parseInt(c.dataset.taskid));});
    document.getElementById('listBody').addEventListener('click',function(e){const r=e.target.closest('.list-row');if(r)showTaskDetail(parseInt(r.dataset.taskid));});
    document.getElementById('closeDetailBtn').addEventListener('click',hideTaskDetail);

 // Sort
 document.getElementById('sortBtn').addEventListener('click',function(){const dd=document.getElementById('sortDropdown'),r=this.getBoundingClientRect();dd.style.top=r.bottom+4+'px';dd.style.left=r.left+'px';dd.style.display=dd.style.display==='none'?'':'none';});
    document.getElementById('sortDropdown').addEventListener('click',function(e){const i=e.target.closest('.dropdown-item');if(!i)return;if(i.dataset.sort){state.sortBy=i.dataset.sort;document.querySelector('.sort-badge').textContent=i.textContent.toUpperCase();}if(i.dataset.direction){state.sortDirection=i.dataset.direction;document.querySelector('.sort-direction').textContent=i.dataset.direction==='desc'?'Newest on top':'Oldest on top';}this.style.display='none';renderView();});

 // Status dropdown
 document.getElementById('detailStatusBtn').addEventListener('click',function(){const dd=document.getElementById('statusDropdown'),r=this.getBoundingClientRect();dd.style.top=r.bottom+4+'px';dd.style.left=r.left+'px';dd.style.display=dd.style.display==='none'?'':'none';});
    document.getElementById('statusDropdown').addEventListener('click',async function(e){const i=e.target.closest('.status-option');if(!i)return;const ns=i.dataset.status,task=state.tasks.find(t=>t.id===state.selectedTaskId);if(task){task.status=ns;try{await ShadowDB.Tasks.update(task);state.tasks=await ShadowDB.Tasks.getAll();}catch(e){}document.getElementById('detailStatusBtn').textContent=ns;document.getElementById('detailStatusBtn').className='status-btn '+statusClass(ns);renderView();}this.style.display='none';});

 // Priority dropdown
 document.getElementById('detailPriority').addEventListener('click',function(){const dd=document.getElementById('priorityDropdown'),r=this.getBoundingClientRect();dd.style.top=r.bottom+4+'px';dd.style.left=r.left+'px';dd.style.display=dd.style.display==='none'?'':'none';});
    document.getElementById('priorityDropdown').addEventListener('click',async function(e){const i=e.target.closest('.priority-option');if(!i)return;const np=i.dataset.priority,task=state.tasks.find(t=>t.id===state.selectedTaskId);if(task){task.priority=np;try{await ShadowDB.Tasks.update(task);state.tasks=await ShadowDB.Tasks.getAll();}catch(e){}document.getElementById('detailPriority').innerHTML='<i class="fa-solid fa-exclamation"></i> '+np;renderView();}this.style.display='none';});
 ...
 // Subtasks
 document.getElementById('subtasksList').addEventListener('click',async function(e){const c=e.target.closest('.check-circle');if(!c||!c.dataset.subtask)return;const task=state.tasks.find(t=>t.id===state.selectedTaskId);if(task){const s=task.subtasks.find(x=>x.id===c.dataset.subtask);if(s){s.done=!s.done;try{await ShadowDB.Tasks.update(task);state.tasks=await ShadowDB.Tasks.getAll();}catch(e){}showTaskDetail(state.selectedTaskId);renderView();}}});
    ...
    document.getElementById('commentInput').addEventListener('keydown',async function(e){if(e.key==='Enter'&&this.value.trim()){const c=this.value.trim();this.value='';if(state.selectedTaskId){try{await ShadowDB.Comments.create({taskId:state.selectedTaskId,text:c,author:'Pradeep'});}catch(e){}}}});
 ...
 // FIX: Settings button - correct selector for right sidebar
 ...
 // FIX: Add Workflow and Playground buttons to header
 ...
 async function init(){
         try{
                     ...
                     state.tasks=await ShadowDB.Tasks.getAll(); state.groups=await ShadowDB.Groups.getAll(); state.tags=await ShadowDB.Tags.getAll();
                     renderSidebar(); updateGroupSelects(); renderView();
                     // FIX: Initialize approval workflow UI if available
