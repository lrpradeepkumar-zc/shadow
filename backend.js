...
const ShadowDB = (function() {
        ...
        const DB_VERSION = 3; // Bumped to 3 to include approval workflow stores
                      ...
                      function openDB() {
                                  return new Promise((resolve, reject) => {
                                                  ...
                                                  request.onsuccess = () => {
                                                                      db = request.result;
                                                                      // Expose db handle for approval-backend.js reuse
                                                                      if (typeof ShadowDB !== 'undefined') ShadowDB._db = db;
                                                                      resolve(db);
                                                  };
                                                  request.onupgradeneeded = (e) => {
                                                                      ...
                                                                      // ===== APPROVAL WORKFLOW STORES =====
                                                                      if (!d.objectStoreNames.contains('approvalRequests')) {
                                                                                              const arStore = d.createObjectStore('approvalRequests', { keyPath: 'id', autoIncrement: true });
                                                                      ...
                                                                      if (!d.objectStoreNames.contains('approvalAuditLogs')) {
                                                                                              const alStore = d.createObjectStore('approvalAuditLogs', { keyPath: 'id', autoIncrement: true });
                                                                      ...
                                                                      if (!d.objectStoreNames.contains('approvalSettings')) {
                                                                                              d.createObjectStore('approvalSettings', { keyPath: 'groupId' });
                                                                      }
                                                  };
                                  });
                      }
                      ...
                      const Tasks = {
                                  ...
                                  getById: (id) => getById(STORES.tasks, id),
                                  ...
                                  complete: async (id) => {
                                                  ...
                                                  if (task) { task.status = 'Completed'; task.completedAt = new Date().toISOString(); return update(STORES.tasks, task); }
                                  },
                                  reopen: async (id) => {
                                                  const task = await getById(STORES.tasks, id);
                      ...
                      const Groups = {
                                  create: (group) => create(STORES.groups, {
                                                  name: group.name || 'New Group', description: group.description || '',
                                                  color: group.color || '#4285f4', type: group.type || 'personal',
                                                  streams: group.streams !== undefined ? group.streams : true, hidden: false,
                                                  categories: group.categories || ['General'],
                                                  statuses: group.statuses || ['Open', 'In Progress', 'Completed'],
                                                  icon: group.icon || null, order: group.order || 0
                                  }),
                      ...
                      // ===== MEMBER OPERATIONS =====
                      const Members = {
                                  create: (member) => create(STORES.members, {
                                                  name: member.name, email: member.email || '',
                                                  role: member.role || 'Member', group: member.group, avatar: member.avatar || null
                                  }),
                      ...
                      const CustomFields = {
                                  create: (field) => create(STORES.customFields, {
                                                  name: field.name, type: field.type || 'Text', group: field.group,
            ...
            return true;
                      ...
                      async function exportAll() {
                                  const data = {};
                                  for (const key of Object.keys(STORES)) { data[key] = await getAll(STORES[key]); }
                                  return data;
                      }
                      ...
                      async function importAll(data) {
                                  for (const key of Object.keys(STORES)) {
                                                  if (data[key]) {
                                                                      ...
                                                                      for (const item of data[key]) { store.add(item); }
                                                  }
                                  }
                                  return true;
                      }
                      ...
                      async function resetAll() {
                                  for (const key of Object.keys(STORES)) { await clearStore(STORES[key]); }
                                  return seed();
                      }
        ...
        Tasks.delete = async (id) => { const r = await _origTaskDelete(id); emit('task:deleted', { id }); emit('data:changed', { entity: 'tasks', action: 'delete' }); return r; };
                      ...
                      return {
                                  ...
                                  Tasks, Groups, Tags, Categories, Members, CustomFields,
                                  Comments, Activity, Settings, on, emit, STORES,
                                  _db: null,
