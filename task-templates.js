...
window.TM = {
  ...
  getFiltered: function() {
    ...
    if (this.activeFilter === 'personal') list = list.filter(function(t) { return t.createdBy === uid; });
    if (this.activeFilter === 'shared') list = list.filter(function(t) { var sw = t.sharedWith || []; return t.createdBy !== uid && sw.some(function(s) { return s.id === uid; }); });
    if (this.activeFilter === 'group') list = list.filter(function(t) { var sw = t.sharedWith || [], mg = tmGetGroups().map(function(g) { return g.id; }); return sw.some(function(s) { return s.type === 'group' && mg.indexOf(s.id) > -1; }); });
...
function tmRenderCard(t) {
  ...
  return '<div class="tm-card' + (TM.previewId === t.id ? ' selected' : '') + '" data-tpl-id="' + t.id + '" data-action="preview">' +
    ...
    '<button class="tm-icon-btn danger" data-tpl-id="' + t.id + '" data-action="delete" title="Delete"><i class="fa-solid fa-trash"></i></button>' : '') +
    '</div></div>';
}
...
function tmRenderLibrary() {
  ...
  return '<div class="tm-lib-sidebar">' +
    ...
    '<button class="tm-new-btn" id="tm-lib-new"><i class="fa-solid fa-plus"></i> New Template</button>' +
    '</div></div>' +
...
function tmBindLibraryEvents(overlay, lib) {
  ...
  lib.querySelectorAll('.tm-filter-tab,.tm-lib-nav-item[data-filter]').forEach(function(tab) { tab.addEventListener('click', function(e) { var f = e.currentTarget.dataset.filter; if (f) { TM.activeFilter = f; TM.previewId = null; tmRefreshLibrary(); } }); });
