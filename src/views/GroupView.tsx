...
export function GroupView() {
  const { currentDisplay, activeGroupId, filters, sortBy, sortDir } = useAppStore()
  ...
  const filtered = useMemo(() => {
    ...
    let result = tasks.filter((t) => {
      if (t.group !== activeGroupId) return false
    ...
    return result
  }, [tasks, filters, activeGroupId, sortBy, sortDir])
  ...
  if (filtered.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-400">
        <span className="text-4xl">👥</span>
        <p className="text-sm">
          No tasks in <strong>{currentGroup?.name ?? 'this group'}</strong>.
        </p>
      </div>
    )
  }
