...
export function SharedWithMeView() {
  ...
  const filtered = useMemo(() => {
    let result = tasks.filter((t) => {
      ...
      // Shared: explicitly in sharedWith array, but NOT created by me or assigned to me
      if (!t.sharedWith.includes(user.id)) return false
  ...
  if (filtered.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-400">
        ...
        <p className="text-sm">No tasks have been shared with you.</p>
      </div>
    )
  }
